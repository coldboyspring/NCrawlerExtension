...
namespace NCrawler {
    public partial class Crawler : DisposableBase {
        ...
        private void ProcessQueue() {
            ...
            var rand = new Random();
            if (MaxThreadSleepTime == 0) {
                MaxThreadSleepTime = 1500;
            }
            while (ThreadsInUse < MaximumThreadCount && WaitingQueueLength > 0) {
                StartDownload();
                Thread.Sleep(rand.Next(MaxThreadSleepTime));
            }
        }
        ...
        private void StartDownload() {
            ...
            webDownloader.Accept = Accept;
            webDownloader.Host = Host;
            webDownloader.KeepAlive = KeepAlive;
            webDownloader.Headers = Headers;
            webDownloader.WebProxy = WebProxy;
